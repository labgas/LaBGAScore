    ...
    for run=1:size(fmriprep_noisefiles,1)
        ...
        % calculate and extract confound regressors
            ...
            elseif strcmpi(spike_def,'CANlab')==1
                ...
                gunzip(fullfile(subjBIDSdir,BIDSimgs{run})); % raw images are needed when spike_def = CANlab, which calls a function that is incompatible with .nii.gz, hence we unzip
                ...
                error('\ninvalid spike_def option %s specified in LaBGAScore_firstlevel_s1_options_dsgn_struct.m, please check before proceeding', spike_def)
            end
        ...
        % add pmods to structures for conditions of interest
                for pmod = 1:size(DSGN.pmods{run},2)
                    ...
                    pmod_demean_run_struct{pmod}.pmod = struct('name',{strcat(DSGN.pmods{run}(pmod),'_demeaned_run')}, ...
                        'param',{{[]}}, ...
                        'poly',{{pmod_polynom}}); % structure for demeaned pmods per run
                    pmod_demean_cond_struct{pmod}.pmod = struct('name',{strcat(DSGN.pmods{run}(pmod),'_demeaned_cond')}, ...
                        'param',{{[]}}, ...
                        'poly',{{pmod_polynom}}); % structure for demeaned pmods per condition
                        if pmod_ortho_off
                    ...
                    while pmod < size(DSGN.pmods{run},2) + 1
                        switch O.trial_type(trial)
                            case DSGN.conditions{run}{pmod}
                               ...
                               if pmod_ortho_off
                                   cond_struct{pmod}.pmod.param{1} = [pmod_demean_cond_struct{pmod}.pmod.param{1},O.pmod_demean_cond(trial)];
                               end
                        end
                    pmod = pmod + 1;
                    end
