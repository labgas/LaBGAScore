...
%  - voxelwise TFCE permutation p-values, UNCORRECTED. Counted against each
%    voxel's own permutation distribution, so they still need correcting for
%    multiple comparisons - FDR downstream, or info.p_FWE below.
%
% info : struct
%  - TFCE_real
%  - TFCE_null
...
%  - TFCE_null_max
%  - p_TFCE_global
%  - p_FWE : voxelwise FWE-corrected p, the standard TFCE correction of
%    Smith & Nichols 2009. Each voxel's observed TFCE is counted against the
%    distribution of the MAXIMUM TFCE over the brain per permutation, so a
%    single threshold controls the familywise error rate across all voxels:
%        p_FWE(v) = ( #{ null_max >= TFCE_real(v) } + 1 ) / (nPerm + 1)
%    Resolution is bounded below by 1/(nPerm+1), so nPerm caps how small a
%    corrected p can be: 1000 permutations cannot go below ~1e-3.
...
info.p_FWE = p_fwe;                        % voxelwise FWE, max-statistic null
