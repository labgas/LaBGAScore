function [poscl, negcl, results_table] = LaBGAScore_region_table(cl, varargin)
% LaBGAScore_region_table  Region table, with the Z ceiling applied only to infinities.
%
%   VENDORED FROM CANLABCORE - RE-CHECK THIS FILE WHEN CANLABCORE IS UPGRADED.
%
%   Source : CanlabCore/CanlabCore/@region/table.m
%   Commit : 8c5992ba04af2d8cbb544006f1ea9987f2a55c09 (2025-08-11)
%   Copied : 2026-09-05, from CanlabCore at 1f6ec616
%
%   WHY THIS COPY EXISTS
%   The upstream method ends its local get_signed_max with
%
%       maxZ = norminv(1 - 1E-12);      % = 7.0345
%       myZ(myZ > maxZ) = maxZ;
%
%   which clips EVERY value above 7.0345, not just the infinite ones its own
%   comment says it is fixing. That is harmless for the t and Z maps CanlabCore
%   itself produces, but TFCE statistics run into the hundreds - TFCE is a LaBGAS
%   addition via group_tfce_from_subject_maps, which CanlabCore has no notion of -
...
%    August 2026: made improvements to retain Z_descrip from input region object. Lukas VO


n_cols = 140;                       % 140 good for HTML reports
sep_str = repmat('_', 1, n_cols);   % see textwrap
Z_descrip = cl.Z_descrip;
val_descrip = cl.val_descrip;
...
if dosep
    ...
    for preg = 1:size(poscl, 2)
        poscl(preg).Z_descrip = Z_descrip;
        poscl(preg).val_descrip = val_descrip;
    end
    
    for nreg = 1:size(negcl, 2)
        negcl(nreg).Z_descrip = Z_descrip;
        negcl(nreg).val_descrip = val_descrip;
    end
...
elseif isempty(region_table)
    ...
    if show_min % show minimum value
        if ~isempty(Z_descrip)
            Z = get_min(cl, 'Z', ['min' Z_descrip]);  % use function because may be empty, handle if so
        else
            Z = get_min(cl, 'Z', 'minZ');
        end
    else % default behavior: max Z / signed max
        if ~isempty(Z_descrip)
            Z = get_signed_max(cl, 'Z', ['max' Z_descrip]);  % use function because may be empty, handle if so
        else
            Z = get_signed_max(cl, 'Z', 'maxZ');
        end
    end
...
% Fix infinite vals.
% LaBGAS CHANGE (see header): upstream clipped every value above the ceiling,
% which flattens any statistic that is not a Z-score. Cap the infinities only -
% which is what this block's comment always said it was for - and cap the
% negative tail as well, which upstream missed.
maxZ = norminv(1 - 1E-12);
myZ(isinf(myZ) & myZ > 0) =  maxZ;
myZ(isinf(myZ) & myZ < 0) = -maxZ;
