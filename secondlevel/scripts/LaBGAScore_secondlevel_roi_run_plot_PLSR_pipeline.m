%% LaBGAScore_secondlevel_roi_run_plot_PLSR_pipeline.m
%
%
% *USAGE*
%
% This script serves as a simple wrapper to run the PLSR (PLS regression)
% neuroimaging pipeline function and its plotting function on fMRI ROI
...
%
% help PLSR_neuroimaging_pipeline
% help plot_PLSR_diagnostics_neuroimaging
%
% or check the READMEs in the LaBGAScore Github repo
%
% https://github.com/labgas/LaBGAScore/blob/main/secondlevel/README_PLSR_neuroimaging_pipeline.md
% https://github.com/labgas/LaBGAScore/blob/main/secondlevel/README_PLSR_plotting.md
%
%
% *OPTIONS*
%
...
% * opts_PLS                struct of PLSR pipeline options (outerK, innerK, nRepeats, maxLV, nPerm, nBoot, learningSteps, seed) — see help PLSR_neuroimaging_pipeline
%
%
% *DEPENDENCIES*
%
...
% -------------------------------------------------------------------------
%
% LaBGAScore_secondlevel_roi_run_plot_PLSR_pipeline.m               v1.0
%
% Derived from LaBGAScore_secondlevel_roi_run_plot_PLS_ENet_pipeline.m v1.4 by
% swapping PLSDA_neuroimaging_pipeline for PLSR_neuroimaging_pipeline (and its
% plotting companion) and removing the Elastic Net arm, which is binary-outcome
% only. Use this script for a CONTINUOUS outcome, the other for a binary one.
% Everything else - ROI feature assembly, fold-wise covariate residualisation,
% atlas handling, figure saving - is shared, so fixes to one usually belong in
% the other as well.
%
...
% (no opts_ENet here - the Elastic Net pipeline is binary-outcome only and is
%  not run by this script; see the ELASTIC NET note further down.)
...
if do_pls
    ...
    for idx = 1:numel(cons2analyze)
        ...
        PLS_results{idx} = PLSR_neuroimaging_pipeline(X_vars{idx},Y_var,opts_PLS);

        [max_varY, idx_LV] = max(PLS_results{idx}.varExplainedY);

        fprintf('\nPlotting latent variable %d explaining %.2f%% of the variance in Y\n\n', idx_LV, max_varY*100);

        PLS_tables{idx} = plot_PLSR_diagnostics_neuroimaging(PLS_results{idx}, [], roiNames, roiatlasFile, ...
            'LV',idx_LV,'TopN',min(p{idx},20),'VIP_thresh',0.8,'stab_thresh',1.5,'MapPrctile',70,'OutPrefix',[num2str(d) '_PLSR'],'RelaxIfEmpty',false,'UnderlayFile',T1_downsample);

        save_all_open_figures_smart(pipeline_resultssubdir,[num2str(d) '_PLSR'],{'fig','svg'},true);

        clear pipeline_resultssubdir

    end
    
    saveplsfilename = fullfile(pipeline_resultsdir,'PLSR.mat');
...
% ELASTIC NET
%
% DELIBERATELY ABSENT. ENet_neuroimaging_pipeline is
% "Elastic Net regularized logistic-style classification (via lassoglm /
% elastic net on a BINARY outcome)": it calls lassoglm(..., 'binomial', ...)
% and coerces the outcome with yNum = double(Y == max(Y)). Handed a continuous
% score that silently binarises it at the maximum, producing a label vector
% that is almost all zeros - a wrong answer with no error message.
%
% There is no regression variant: quickCV_ENet, quickCV_ENet_PR and
% bootstrapOOB_ENet are all components of that classification pipeline.
% Writing one (lassoglm with 'normal' rather than 'binomial', reusing the
% surrounding CV / permutation / bootstrap machinery) is real work and needs
% its own validation; use LaBGAScore_secondlevel_roi_run_plot_PLS_ENet_pipeline
% for binary outcomes until then.
