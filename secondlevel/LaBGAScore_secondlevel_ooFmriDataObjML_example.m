...
innercv = @(X,Y) cvpartition2(length(Y), 'GroupKFold', 2, 'Group', X.metadata.participant_id);
...
outercv = @(X,Y) cvpartition2(length(Y), 'GroupKFold', 3, 'Group', X.metadata_table.participant_id);
