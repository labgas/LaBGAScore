...
#### 4. Run the script wrapped in a datalad run command

You can obviously run that script in Matlab and then `datalad save` the output it creates in the derivatives subdataset, but like `KUL_dcm2bids.sh` above, we can also wrap a Matlab command in a `datalad run` command:

```bash
cd /data/proj_erythritol/proj_erythritol_4a
datalad run -m "run ery_4a_prep_s1_write_events_tsv.m on all subjects" -i "sourcedata/sub-*/logfiles" -o "BIDS/sub-*/*events.tsv" -o "BIDS/phenotype.tsv" "matlab -batch \"ery_4a_prep_s1_write_events_tsv\""
```

This has the advantage of being more reproducible (with `datalad rerun`) as well as automatically neatly saving everything to the relevant subdataset and the superdataset.
...
### 3. Run the script wrapped in a datalad run command

You can obviously run the script in Matlab and then `datalad save` the output it creates in the derivatives subdataset from your Linux command line, but like `KUL_dcm2bids.sh` above, we can also wrap a Matlab command in a `datalad run` command:

```bash
cd /data/proj_erythritol/proj_erythritol_4a
datalad run -m "run ery_4a_prep_s2_smooth.m on all subjects" -i "derivatives/fmriprep/sub-*/func/*preproc_bold.nii.gz" -o "derivatives/fmriprep/sub-*/func/s6*" -o "derivatives/fmriprep/sub-*/func/*.mat" "matlab -batch \"ery_4a_prep_s2_smooth\""
```
...
First-level analysis can be run flexibly using the three scripts in the [`firstlevel` directory of the LaBGAScore Github repo](https://github.com/labgas/LaBGAScore/tree/main/firstlevel). A parallel phMRI-specific variant of these scripts (`s1a`/`s1b`/`s2a`/`s2b`/`s3b`/`s3c`) exists for multisession/multitask and pharmacological-challenge designs; see the LaBGAScore [README.md](https://github.com/labgas/LaBGAScore/blob/main/README.md) for details.

The scripts themselves are extensively annotated and documented, hence we refer to their documentation here.
...
The scripts themselves are extensively annotated and documented, hence we refer to their documentation here. The table below gives a short description of each of the three core scripts, based on the *USAGE* section of their Matlab header documentation.
...
### 5. Run the script wrapped in a datalad run command

Like all scripts in the LaBGAScore repo, it should be run from the root directory of your superdataset! You only need to run the second script, since it calls the first and third under the hood!

**NOTE:** do not use the `matlab -batch` option in this case, since it will cause publication of the diagnostic report (the third script) to fail!

```bash
cd /data/proj_erythritol/proj_erythritol_4a
datalad run -m "run ery_4a_firstlevel_m1_s2_fit_model.m on all subjects" -i "derivatives/fmriprep/sub-*/func/*" -o "firstlevel/model_1_conds_pmods/*" "matlab -r \"ery_4a_firstlevel_m1_s2_fit_model\""
```
