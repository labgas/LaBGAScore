...
% varNames  = [{'subject'} (atlas.labels)];
varNames  = [{'subject'} (roi_atlas.labels)]; % lixin may26 2024
varTypes(1,1)           = {'string'};
...
for subj = 1:nr_subjects
    ...
    for voi = 1:nr_VOIS
        ...
        if figures_on == 1 || save_figures == 1 
           ...
           title(['Logan plot: ' LCN_string(VOIname)]);
           % set(hfig4,'NumberTitle','off')
           set(hfig3,'Name',[subjectname '- Logan\_input']);
           if save_figures == 1
              cd(dir_figures);
              saveas(hfig3,['fig_Logan_input_VOI_' VOIname '.fig']);
           else 
              pause
           end
        else
        ...
        if figures_on == 1 || save_figures == 1 
           ...
           if save_figures == 1
              cd(dir_figures);
              saveas(hfig4,['fig_2T4k_Vb_' VOIname '.fig']);
           else 
              pause
           end
        end
