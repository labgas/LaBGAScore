function [C_calc] = LCN_calc2_model_2T4k(params, midscantimes, frameduration, CA_FINETIMES,FINETIMES,calc_option,STEP)
%
% calculates the output value for the model_2T4k_Vbfixed (with parameters params)
%
% params: values for [K1,k2,k3,k4]
...
% history: 	
%__________________________________________________________________________
% @(#)LCN_calc2_model_2T4k.m	      v0.1        last modified: 2024/11/25
...
C_calc = factor .* ((k3 + k4 - alfa1) * exp(-alfa1 * FINETIMES) .* int_finetimes1 - (k3 + k4 - alfa2) * exp(-alfa2 * FINETIMES) .* int_finetimes2);
