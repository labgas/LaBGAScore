function [C_calc] = LCN_calc_model_2T4k_vasc2k(params,midscantimes,frameduration,CA_FINETIMES,CA_WB_FINETIMES,FINETIMES,calc_option,STEP)
%
...
%
% params: values for [K1,k2,k3,k4,Vb,K1v,k2v]
...
% history: 	
%__________________________________________________________________________
% @(#)LCN_calc_model_2T4k_vasc2k.m	   v0.1       last modified: 2025/06/24
...
int_finetimes3 = STEP*cumsum(CA_WB_FINETIMES.*exp(k2v*FINETIMES));

C_calc = (1-Vb)*(phi1*exp(-theta1*FINETIMES).*int_finetimes1+phi2*exp(-theta2*FINETIMES).*int_finetimes2)+Vb*(CA_WB_FINETIMES+K1v*exp(-k2v*FINETIMES).*int_finetimes3);
if calc_option == 1
